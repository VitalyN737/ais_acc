...
async function getManifest() {
  ...
  try {
    ...
    console.warn("Failed to fetch manifest.json from GitHub, using embedded manifest.", e);
...
export async function fetchCollection(collectionName: string) {
  ...
  const files = manifest[collectionName] || [];
  
  if (files.length === 0) {
    console.warn(`No files found for collection ${collectionName} in manifest.`);
  }

  const dataPromises = files.map(file => fetchJson(`${collectionName}/${file}`));
