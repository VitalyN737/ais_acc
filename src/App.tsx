...
const initialMenu = { items: [] as Array<{ name: string; link: string }> };

const normalizeMenuItems = (menu: any): Array<{ name: string; link: string }> => {
  ...
  return rawItems
    ...
    .filter((item: { name: string; link: string } | null): item is { name: string; link: string } => Boolean(item));
};
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (root) {
      root.style.overflowY = isMenuOpen ? 'hidden' : 'auto';
    }
  }, [isMenuOpen]);

  const menuItems = normalizeMenuItems(menuData);
